...
class Gitlab(object):
    ...
    def build_url(self, suffix):
        return 'https://{}:{}@{}/{}.git'.format(self.user, self.token, self.url, suffix)
    ...
    def get_projects(self, group_id):
        repo_project = {}
        for project in self.get("groups/{}/projects".format(group_id)):
            repo_project[project['name']] = project
        return repo_project
    ...
    def download_projects(self, projects, dir):
        for project in projects:
            print("Download project", project, projects[project]['id'], projects[project]['http_url_to_repo'])
            self.download_project(self.build_url(projects[project]['path_with_namespace']),
                                  os.path.normpath(os.path.join(dir, projects[project]['name'])))
        ...
        for group_name in groups:
            ...
            if group_pattern in group_name:
                ...
                for project_name in projects:
                    ...
                    self.delete(os.path.join("projects", project_id))
                    print('-', '-', 'Project deleted', project_name, project_id)
                self.delete(os.path.join("groups", group_id))
                print('-', 'Group deleted')
            else:
    ...
    def h_list_project(self, group_name=None):
        ...
        for grp_name in group_ids:
            projects = self.get_projects(group_ids[grp_name])
            if (group_name not in ('', None) and grp_name == group_name) or group_name in ('', None):
                print(group_ids[grp_name], grp_name, len(projects))
                for project in projects:
                    print('  - ', projects[project]['id'], project, projects[project]['path_with_namespace'])
            ...
            if 'id' not in group:
                ...
                for project_folder in fl.get_folders(os.path.join(path, group_folder)):
                    ...
                    res = self.sync(
                        ...
                        'Auto-commit',
                        self.build_url('{}/{}'.format(group_name, project_name))
                    )
                    for action in res:
                        print('-', '-', '-', action)
    ...
    def h_download(self, dir, group_name='', project_name=''):
        ...
        if project_name not in ('', None) and group_name not in('', None):
            self.download_project(self.build_url('{}/{}'.format(group_name, project_name)),
                                  os.path.join(dir, group_name, project_name))
            return

        if group_name not in('', None):
            group = self.get_group(group_name)
            if group and group not in('', None) and 'id' not in group:
                print('Group not found', group_name)
            else:
                print('Scanning GitLab project...')
                projects = self.get_projects(group['id'])
                for project in projects:
                    print("Download project", project, projects[project]['id'], projects[project]['ssh_url_to_repo'])
                    self.download_project(self.build_url(projects[project]['path_with_namespace']),
                                          os.path.normpath(os.path.join(dir, group_name, project)))
            return
        else:
            group_ids = self.get_group_ids()
            for grp_name in group_ids:
                ...
                for project in projects:
                    print("Download project", project, projects[project]['id'], projects[project]['ssh_url_to_repo'])
                    self.download_project(self.build_url(projects[project]['path_with_namespace']),
                                          os.path.normpath(os.path.join(dir, grp_name, project)))
